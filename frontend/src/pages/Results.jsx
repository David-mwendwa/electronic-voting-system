...
import { FiArrowLeft, FiBarChart2, FiAward, FiUsers, FiCalendar, FiClock, FiHome, FiDownload } from 'react-icons/fi';
...
const Results = () => {
  ...
  const generateReport = () => {
    ...
    doc.text(`Election ID: ${election.id}`, 15, 45);
    doc.text(`Total Voters: ${election.totalVoters.toLocaleString()}`, 15, 50);
    doc.text(`Votes Cast: ${totalVotes.toLocaleString()}`, 15, 55);
    doc.text(`Turnout: ${turnout.toFixed(1)}%`, 15, 60);
    
    // Add date and time
    const now = new Date();
    doc.text(`Report generated on: ${now.toLocaleDateString()} at ${now.toLocaleTimeString()}`, pageWidth - 15, 45, { align: 'right' });
    ...
    doc.text(isTie ? 'Election Results in a Tie' : 'Election Winner', 15, finalY);
    
    doc.setFontSize(12);
    doc.setTextColor(75, 85, 99);
    if (isTie) {
      const tiedCandidates = results.filter(r => r.votes === leadingCandidate.votes);
      doc.text(`The following candidates are tied with ${leadingCandidate.votes} votes each:`, 15, finalY + 10);
      
      tiedCandidates.forEach((candidate, index) => {
        doc.text(`${index + 1}. ${candidate.name} (${candidate.party || 'Independent'})`, 20, finalY + 20 + (index * 5));
      });
    } else {
      doc.text(`The winner is ${leadingCandidate.name} with ${leadingCandidate.percentage.toFixed(1)}% of the votes.`, 15, finalY + 10);
    }
  ...
  useEffect(() => {
    const loadElection = async () => {
      ...
      const votes = Object.values(foundElection.results || {}).reduce((sum, count) => sum + count, 0);
      
      setElection(foundElection);
        
      // Process results
      const resultsData = foundElection.candidates.map(candidate => ({
        ...candidate,
        votes: foundElection.results?.[candidate.id] || 0
      }));
    ...
    loadElection();
  }, [electionId, navigate, getElectionById]);
  ...
  const turnout = totalVotes / election.totalVoters * 100;
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-4">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              <div className="mb-4 md:mb-0">
                <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">{election.title}</h1>
                <p className="mt-1 text-sm sm:text-base text-gray-600 line-clamp-2">{election.description}</p>
              ...
              <div className="mt-3 md:mt-0 bg-primary-50 p-3 sm:p-4 rounded-lg">
                <div className="text-xs sm:text-sm font-medium text-gray-600 mb-1">Turnout</div>
                <div className="flex items-baseline">
                  <span className="text-2xl sm:text-3xl font-bold text-primary-700">{turnout.toFixed(1)}%</span>
                  <span className="ml-2 text-xs sm:text-sm text-gray-500">
                    ({totalVotes} of {election.totalVoters} votes)
                  </span>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="lg:col-span-2 space-y-4 sm:space-y-6">
                ...
                {results.map((candidate, index) => (
                  <div key={candidate.id} className="space-y-1.5">
                    <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between">
                      <div className="flex items-center">
                        <div className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full ${chartColors[index % chartColors.length]} mr-2`}></div>
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                <h2 className="text-base sm:text-lg font-medium text-gray-900">Detailed Results</h2>
              </div>
                ...
                <div className="inline-block min-w-full align-middle px-2 sm:px-0">
                  <table className="min-w-full divide-y divide-gray-200 text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        <th scope="col" className="px-3 py-2.5 sm:px-4 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                          Candidate
                        </th>
                        <th scope="col" className="px-2 py-2.5 sm:px-4 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                          Party
                        </th>
                        <th scope="col" className="px-2 py-2.5 sm:px-4 sm:py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                          Votes
                        </th>
                        <th scope="col" className="px-2 py-2.5 sm:px-4 sm:py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                          %
                      ...
                      {results.map((candidate, index) => (
                        <tr key={candidate.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                          <td className="px-3 py-2.5 sm:px-4 sm:py-3 whitespace-nowrap">
                            <div className="flex items-center">
                              ...
                              <div className="ml-2 sm:ml-3">
                                <div className="text-xs sm:text-sm font-medium text-gray-900 line-clamp-1">{candidate.name}</div>
                          ...
                          <td className="px-2 py-2.5 sm:px-4 sm:py-3 whitespace-nowrap text-right text-xs sm:text-sm font-medium">
                            <span className={`px-1.5 py-0.5 sm:px-2 sm:py-1 inline-flex text-[10px] sm:text-xs leading-4 font-semibold rounded-full ${
                              index === 0 && !isTie ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                            }`}>
                              {candidate.percentage.toFixed(1)}%
                            </span>
                          </td>
          ...
          <div className="space-y-4 sm:space-y-6">
              ...
              <div className="p-4 sm:p-5">
                {isTie ? (
                  <div className="text-center">
                    ...
                    <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-1.5">Tied Election</h3>
                    <p className="text-xs sm:text-sm text-gray-600 mb-3">
                      Top {results.filter(r => r.votes === leadingCandidate.votes).length} candidates tied with {leadingCandidate.votes} votes each.
                    </p>
                    <div className="space-y-2">
                      {results
                        .filter(r => r.votes === leadingCandidate.votes)
                        .map((candidate, index) => (
                          <div key={candidate.id} className="flex items-center p-2 sm:p-3 bg-gray-50 rounded-lg">
                            <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full ${chartColors[index % chartColors.length]} flex items-center justify-center text-white font-medium text-xs sm:text-sm`}>
                              {candidate.name.charAt(0)}
                            </div>
                            <div className="ml-2 sm:ml-3 min-w-0">
                              <div className="text-xs sm:text-sm font-medium text-gray-900 truncate">{candidate.name}</div>
                              <div className="text-[10px] sm:text-xs text-gray-500 truncate">{candidate.party || 'Independent'}</div>
                            </div>
                  ...
                  <div className="text-center">
                    ...
                    <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-1">{leadingCandidate.name}</h3>
                    <p className="text-sm text-gray-600 mb-3 sm:mb-4">{leadingCandidate.party || 'Independent'}</p>
                    <div className="bg-green-50 rounded-lg p-3">
                      <div className="text-xs sm:text-sm font-medium text-gray-600 mb-1">Votes Received</div>
                      <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-center">
                        <span className="text-xl sm:text-2xl font-bold text-green-700">{leadingCandidate.votes.toLocaleString()}</span>
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                <h2 className="text-base sm:text-lg font-medium text-gray-900">Election Stats</h2>
              </div>
              <div className="p-4 sm:p-5 space-y-3 sm:space-y-4">
                <div>
                  <div className="text-xs sm:text-sm font-medium text-gray-500">Total Votes Cast</div>
                  <div className="mt-0.5 text-lg sm:text-xl font-semibold text-gray-900">{totalVotes.toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-xs sm:text-sm font-medium text-gray-500">Voter Turnout</div>
                  <div className="mt-0.5">
                    <div className="text-lg sm:text-xl font-semibold text-gray-900">{turnout.toFixed(1)}%</div>
                    <div className="text-xs sm:text-sm text-gray-500 mt-0.5">
                      {totalVotes.toLocaleString()} of {election.totalVoters.toLocaleString()} voters
                    </div>
                  </div>
                ...
                <div>
                  <div className="text-xs sm:text-sm font-medium text-gray-500 mb-1">Voting Period</div>
        ...
        <div className="mt-6 sm:mt-8 text-center text-xs text-gray-500 pb-4 sm:pb-6">
          <p>Results as of {new Date().toLocaleDateString()} at {new Date().toLocaleTimeString()}</p>
